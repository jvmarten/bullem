...
export function decideCFRWithSearch(
  ...
  // Safety adjustments for multiplayer (3+ players).
  // The trained strategy is well-converged for heads-up (p2 bucket) but
  // under-converged for multiplayer. Card-aware adjustments compensate
  // for the weak multiplayer strategy without hurting 1v1 performance.
  if (activePlayers >= 3) {
 ...
 *
 * For non-raise actions (bull/true/pass), uses the standard action mapper.
 * For raise actions, uses hybrid card-aware raise generation that scores
 * candidates by plausibility instead of picking random ranks.
 */
function sampleAndMap(
  ...
  if (total > 0) {
    ...
    for (const action of legalActions) {
      cumulative += probs.get(action) ?? 0;
      if (r <= cumulative) {
        return hybridMapAction(action, state, botCards, totalCards);
      }
    }
  }

  // Fallback: use last legal action
  return hybridMapAction(
    legalActions[legalActions.length - 1]!,
    state, botCards, totalCards,
  );
}

// ── Hybrid card-aware raise generation ────────────────────────────────

/** Actions that represent raises (truthful or bluff). */
const RAISE_ACTIONS = new Set([
  AbstractAction.TRUTHFUL_LOW, AbstractAction.TRUTHFUL_MID, AbstractAction.TRUTHFUL_HIGH,
  AbstractAction.BLUFF_SMALL, AbstractAction.BLUFF_MID, AbstractAction.BLUFF_BIG,
]);

/**
 * Hybrid action mapper: uses standard mapper for bull/true/pass,
 * but card-aware plausibility-scored raise generation for raise actions.
 *
 * The CFR strategy decides WHETHER to raise — this function decides WHAT
 * to raise with, using Bayesian probability instead of random ranks.
 */
function hybridMapAction(
  action: AbstractAction,
  state: ClientGameState,
  botCards: Card[],
  totalCards: number,
): BotAction {
  // Non-raise actions use the standard mapper (bull/true/pass are straightforward)
  if (!RAISE_ACTIONS.has(action)) {
    return mapAbstractToConcreteAction(action, state, botCards, totalCards);
  }

  // Determine if this is a truthful or bluff raise, and the magnitude
  const isTruthful = action === AbstractAction.TRUTHFUL_LOW
    || action === AbstractAction.TRUTHFUL_MID
    || action === AbstractAction.TRUTHFUL_HIGH;
  const magnitude: 'low' | 'mid' | 'high' = action === AbstractAction.TRUTHFUL_LOW || action === AbstractAction.BLUFF_SMALL
    ? 'low' : action === AbstractAction.TRUTHFUL_MID || action === AbstractAction.BLUFF_MID
      ? 'mid' : 'high';

  const { currentHand, roundPhase } = state;

  // Try card-aware raise generation
  const raise = isTruthful
    ? findTruthfulRaise(botCards, currentHand, totalCards, magnitude)
    : findPlausibleBluffRaise(botCards, currentHand, totalCards, magnitude);

  if (raise) {
    if (roundPhase === RoundPhase.LAST_CHANCE) {
      return { action: 'lastChanceRaise', hand: raise };
    }
    return { action: 'call', hand: raise };
  }

  // No viable raise found — fall back to bull (with self-verify safety)
  if (roundPhase === RoundPhase.LAST_CHANCE) {
    return { action: 'lastChancePass' };
  }
  if (currentHand && HandChecker.exists(botCards, currentHand)) {
    return { action: 'true' };
  }
  return { action: 'bull' };
}
...
/**
 * Find a truthful raise — a hand the bot can partially or fully back with
 * its own cards. Returns the appropriate tier based on magnitude.
 *
 * 'low' = lowest valid raise, 'mid' = middle, 'high' = highest available.
 */
function findTruthfulRaise(
  botCards: Card[],
  currentHand: HandCall | null,
  totalCards: number,
  ...
  // Two pair (from ranks we hold)
  const heldRanks = [...rankCounts.keys()];
  if (heldRanks.length >= 2) {
    for (let i = 0; i < heldRanks.length; i++) {
      for (let j = i + 1; j < heldRanks.length; j++) {
        const [hi, lo] = RANK_VALUES[heldRanks[i]!] > RANK_VALUES[heldRanks[j]!]
          ? [heldRanks[i]!, heldRanks[j]!] : [heldRanks[j]!, heldRanks[i]!];
        candidates.push({ type: HandType.TWO_PAIR, highRank: hi, lowRank: lo });
      }
    }
  }

  // Flush (from suits we hold 2+)
  ...
  // Three of a kind (from ranks we hold)
  for (const [rank, count] of rankCounts) {
    if (count >= 2) {
  ...
  // Straight (based on our card values)
  {
    const vals = [...new Set(botCards.map(c => RANK_VALUES[c.rank]))].sort((a, b) => a - b);
    if (vals.length >= 2) {
      const highVal = Math.min(vals[vals.length - 1]! + 2, 14);
      if (highVal >= 5) {
        const highRank = ALL_RANKS.find(r => RANK_VALUES[r] === highVal);
        if (highRank) candidates.push({ type: HandType.STRAIGHT, highRank });
      }
    }
  }

  // Full house (if we have trips or pairs)
  for (const [threeRank, count] of rankCounts) {
    if (count >= 2) {
      for (const twoRank of heldRanks) {
        if (twoRank !== threeRank) {
  ...
  // Four of a kind (from ranks we hold 2+)
  for (const [rank, count] of rankCounts) {
    if (count >= 2) {
  ...
  // Filter: must be a valid raise AND plausible given total cards
  const maxType = getMaxPlausibleType(totalCards);
  const valid = candidates.filter(h => {
    if (h.type > maxType) return false;
    if (currentHand && !isHigherHand(h, currentHand)) return false;
    return true;
  });
  ...
  valid.sort((a, b) => {
    ...
    const aRank = getHandPrimaryRankValue(a);
    const bRank = getHandPrimaryRankValue(b);
    return aRank - bRank;
  });

  // Deduplicate
  const unique = deduplicateHands(valid);
  if (unique.length === 0) return null;

  const idx = magnitude === 'low' ? 0
    : magnitude === 'mid' ? Math.floor(unique.length / 2)
      : unique.length - 1;
  return unique[idx]!;
}

/**
 * Find a plausibility-scored bluff raise. Instead of picking random ranks,
 * generates all viable raise candidates and scores them by:
 * - Bayesian probability of the hand existing (semi-bluffs preferred)
 * - Escalation penalty (prefer minimal type jumps)
 * - Narrative consistency (ranks already mentioned in round history)
 */
function findPlausibleBluffRaise(
  botCards: Card[],
  currentHand: HandCall | null,
  totalCards: number,
  magnitude: 'low' | 'mid' | 'high',
): HandCall | null {
  const candidates: { hand: HandCall; score: number }[] = [];
  const rankCounts = new Map<Rank, number>();
  for (const c of botCards) {
    rankCounts.set(c.rank, (rankCounts.get(c.rank) ?? 0) + 1);
  }
  ...
  // Determine target type range based on magnitude
  const minOffset = magnitude === 'low' ? 0 : magnitude === 'mid' ? 1 : 2;
  const maxOffset = magnitude === 'low' ? 1 : magnitude === 'mid' ? 2 : 4;

  // Generate candidates for each type in range
  for (let offset = minOffset; offset <= maxOffset; offset++) {
    const targetType = currentType + offset;
    if (targetType < HandType.HIGH_CARD || targetType > maxType) continue;

    const typeCandidates = generateRaiseCandidatesForType(
      targetType as HandType, botCards, rankCounts, currentHand, totalCards,
    );

    for (const hand of typeCandidates) {
      // Score by plausibility × escalation penalty
      const plausibility = computeHandExistsProbability(hand, botCards, totalCards);
      const typeGap = Math.max(0, hand.type - currentType - 1);
      const escalationPenalty = Math.pow(0.85, typeGap);

      // Bonus for semi-bluffs (we hold some of the claimed cards)
      const semiBluffBonus = getSemiBluffBonus(hand, botCards, rankCounts);

      const score = plausibility * escalationPenalty * semiBluffBonus;
      if (score > 0.02) { // Minimum plausibility threshold
  ...
  if (candidates.length === 0) {
    // Try minimum raise as fallback
    if (currentHand) {
      const minRaise = getMinimumRaise(currentHand);
      if (minRaise && minRaise.type > currentType && minRaise.type <= maxType) {
        const p = computeHandExistsProbability(minRaise, botCards, totalCards);
        if (p > 0.02) return minRaise;
      }
    }
    return null;
  }
  ...
  // Pick from top 3 with weighted random (higher score = more likely)
  const topN = Math.min(3, candidates.length);
  let totalScore = 0;
  for (let i = 0; i < topN; i++) totalScore += candidates[i]!.score;
  const r = Math.random() * totalScore;
  let cumulative = 0;
  for (let i = 0; i < topN; i++) {
    cumulative += candidates[i]!.score;
    if (r <= cumulative) return candidates[i]!.hand;
  }
  return candidates[0]!.hand;
}

/**
 * Generate concrete raise candidates for a specific hand type.
 * Uses the bot's own cards to prefer ranks it holds (semi-bluffs).
 */
function generateRaiseCandidatesForType(
  type: HandType,
  botCards: Card[],
  ...
  const candidates: HandCall[] = [];
  const heldRanks = [...rankCounts.keys()];
  // Ranks sorted by how many we hold (descending), then by value
  const rankedByHolding = ALL_RANKS
    .map(r => ({ rank: r, count: rankCounts.get(r) ?? 0, value: RANK_VALUES[r] }))
    .sort((a, b) => b.count - a.count || b.value - a.value);

  switch (type) {
    case HandType.HIGH_CARD:
      // Prefer ranks we DON'T hold (like humans do) but that are high
      for (const r of ALL_RANKS) {
        if (!rankCounts.has(r) && RANK_VALUES[r] >= 8) {
          candidates.push({ type: HandType.HIGH_CARD, rank: r });
        }
      }
      break;

    case HandType.PAIR:
      // Prefer ranks we hold (semi-bluff: we have 1, need 1 more)
      for (const { rank, count } of rankedByHolding) {
        candidates.push({ type: HandType.PAIR, rank });
    ...
    case HandType.FLUSH: {
      if (totalCards < 8) break;
      const suitCounts = new Map<Suit, number>();
      for (const c of botCards) suitCounts.set(c.suit, (suitCounts.get(c.suit) ?? 0) + 1);
      // Prefer suits we have the most of
      const sortedSuits = [...suitCounts.entries()].sort((a, b) => b[1] - a[1]);
      for (const [suit] of sortedSuits) {
        candidates.push({ type: HandType.FLUSH, suit });
      }
      // Also include suits we don't hold (pure bluff option)
      for (const suit of ALL_SUITS) {
    ...
    case HandType.THREE_OF_A_KIND:
      if (totalCards < 6) break;
      // Prefer ranks we hold (semi-bluff)
      for (const { rank, count } of rankedByHolding) {
        if (count >= 1) {
          candidates.push({ type: HandType.THREE_OF_A_KIND, rank });
        }
    ...
    case HandType.FULL_HOUSE:
      if (totalCards < 8) break;
      // Prefer ranks we hold 2+ of for the triple
      for (const { rank: threeRank, count } of rankedByHolding) {
        if (count < 1) continue;
        for (const twoRank of heldRanks) {
          if (twoRank !== threeRank) {
            candidates.push({ type: HandType.FULL_HOUSE, threeRank, twoRank });
    ...
    case HandType.FOUR_OF_A_KIND:
      if (totalCards < 10) break;
      // Only claim four-of-a-kind for ranks we hold at least 1 of
      for (const { rank, count } of rankedByHolding) {
        if (count >= 1) {
          candidates.push({ type: HandType.FOUR_OF_A_KIND, rank });
        }
  ...
  // Filter to valid raises above current hand
  return candidates.filter(h => !currentHand || isHigherHand(h, currentHand));
}

/**
 * Bonus multiplier for semi-bluffs (hands where the bot holds some of
 * the required cards, making the claim more likely to be true).
 */
function getSemiBluffBonus(
  hand: HandCall,
  botCards: Card[],
  rankCounts: Map<Rank, number>,
): number {
  switch (hand.type) {
    case HandType.HIGH_CARD:
      return rankCounts.has(hand.rank) ? 1.5 : 1.0;
    case HandType.PAIR:
    case HandType.THREE_OF_A_KIND:
    case HandType.FOUR_OF_A_KIND: {
      const count = rankCounts.get(hand.rank) ?? 0;
      return 1.0 + count * 0.3; // 1.0 for 0, 1.3 for 1, 1.6 for 2, etc.
    }
    case HandType.TWO_PAIR: {
      const hiCount = rankCounts.get(hand.highRank) ?? 0;
      const loCount = rankCounts.get(hand.lowRank) ?? 0;
      return 1.0 + (hiCount + loCount) * 0.2;
    }
    case HandType.FLUSH: {
      const suitCount = botCards.filter(c => c.suit === hand.suit).length;
      return 1.0 + suitCount * 0.2;
    }
    case HandType.FULL_HOUSE: {
      const threeCount = rankCounts.get(hand.threeRank) ?? 0;
      const twoCount = rankCounts.get(hand.twoRank) ?? 0;
      return 1.0 + threeCount * 0.25 + twoCount * 0.15;
    }
    case HandType.STRAIGHT:
    case HandType.STRAIGHT_FLUSH: {
      const highVal = RANK_VALUES[hand.highRank];
      ...
      for (let v = highVal - 4; v <= highVal; v++) {
        ...
        if (botCards.some(c => RANK_VALUES[c.rank] === actualV)) held++;
      }
      return 1.0 + held * 0.2;
    }
    default:
...
/** Get the maximum plausible hand type given total cards in play. */
function getMaxPlausibleType(totalCards: number): HandType {
  for (let t = HandType.ROYAL_FLUSH; t >= HandType.HIGH_CARD; t--) {
    if (totalCards >= (MIN_CARDS_FOR_PLAUSIBLE[t] ?? 999)) {
...
/** Get primary rank value for sorting hands within a type. */
function getHandPrimaryRankValue(hand: HandCall): number {
  if ('rank' in hand && hand.rank) return RANK_VALUES[hand.rank];
  if ('highRank' in hand && hand.highRank) return RANK_VALUES[hand.highRank];
  if ('threeRank' in hand && hand.threeRank) return RANK_VALUES[hand.threeRank];
  return 0;
}

/** Remove duplicate hands (same type + same ranks/suit). */
function deduplicateHands(hands: HandCall[]): HandCall[] {
  const seen = new Set<string>();
  const result: HandCall[] = [];
