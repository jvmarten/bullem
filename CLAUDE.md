...
- **Native Apps:** iOS and Android via Capacitor — thin WebView shells. Server-side changes take effect without App Store updates
...
Bull 'Em uses [Fly.io](https://fly.io) for hosting, with a multi-stage Docker build.
...
2. Create a Fly app: `fly apps create your-app-name`
3. Configure secrets (see `.env.example` for required environment variables)
4. Deploy: `fly deploy`
...
1. **`DEFAULT_BOT_PROFILE_CONFIG`** = exact evolved champion values
...
- Have database accounts for ranked play
...
Training scripts live in the `training` workspace:
