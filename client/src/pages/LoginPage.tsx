...
/** The production server origin — needed to build absolute OAuth URLs for Browser.open(). */
const SERVER_ORIGIN = 'https://bullem.cards';
...
/**
 * OAuth button that uses Browser.open() on native (Safari overlay) so the
 * WKWebView stays on the React app and the appUrlOpen listener can catch
 * the bullem:// deep link callback. On web, renders a normal <a> tag.
 */
function OAuthButton({
  ...
  const href = native
    ? `${SERVER_ORIGIN}${path}?source=capacitor`
    : `${API_BASE}${path}`;

  const handleNativeClick = useCallback((e: React.MouseEvent) => {
    e.preventDefault();
    // Open in Safari overlay — keeps WKWebView intact so appUrlOpen listener works
  ...
  return (
    <a
      href={href}
      onClick={native ? handleNativeClick : undefined}
      className={className}
      style={style}
    ...
    <Layout>
      <div className="flex flex-col items-center pt-12 max-w-sm mx-auto">
        ...
        {/* OAuth buttons — work for both sign in and registration.
           On Capacitor native, use Browser.open() to open OAuth in a Safari
           overlay (SFSafariViewController) instead of navigating the WKWebView
           away from the React app. This keeps the appUrlOpen listener alive
           so the bullem:// deep link callback can be handled properly. */}
        <OAuthButton
          provider="google"
