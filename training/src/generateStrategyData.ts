...
// Generate the reverse mapping so runtime code can translate abbreviated keys
const reverseMap = Object.entries(ACTION_ABBREVIATIONS)
 ...
 *
 * Trained with CFR+ (regret clamping + t² strategy weighting) for 2M iterations.
 * Standard config only (no jokers, classic LCM). Variant configs fall back
 * to the context-aware heuristic at runtime.
 * Action names are abbreviated to reduce bundle size.
 *
...
/** Abbreviated action name → full AbstractAction name. */
export const ACTION_EXPAND: Record<string, string> = {
${reverseMap},
};
