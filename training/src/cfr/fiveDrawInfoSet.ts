/**
 * Information set abstraction for 5 Draw CFR training.
 *
 * 5 Draw is a simpler game than standard Bull 'Em:
 * - Always 2 players, 5 cards each, 10 total
 * - Actions: raise (call higher hand) or pass
 * - No bull/true phases
 * - Pass ends the round immediately
 *
 * The info set is therefore much more compact than the standard game.
 */

import type { Card, HandCall, Rank, Suit } from '@bull-em/shared';
import { HandType, RANK_VALUES } from '@bull-em/shared';

// ── Abstract action space for 5 Draw ────────────────────────────────

/**
 * Abstract actions for 5 Draw CFR.
 * Same as standard except no BULL or TRUE — replaced by PASS.
 */
export enum FiveDrawAction {
  ...
  BLUFF_MID = 'bluff_mid',
  BLUFF_BIG = 'bluff_big',
  /** Decline to raise — ends the round. */
  PASS = 'pass',
}

export const ALL_FIVE_DRAW_ACTIONS: readonly FiveDrawAction[] = Object.values(FiveDrawAction);

// ── Legal action determination ──────────────────────────────────────

/**
 * Determine legal actions for the current 5 Draw decision point.
 */
export function getFiveDrawLegalActions(
  currentHand: HandCall | null,
): FiveDrawAction[] {
  if (!currentHand) {
    // Opening move — must call a hand (no pass)
    return [
  ...
  // Subsequent move — can raise or pass
  return [
    FiveDrawAction.PASS,
    FiveDrawAction.TRUTHFUL_LOW,
    FiveDrawAction.TRUTHFUL_MID,
...
// ── Hand strength bucketing (richer for 5 cards) ────────────────────

/**
 * With 5 cards, we can bucket more precisely than the standard 3-bucket
 * system. 4 buckets capture the key strength distinctions.
 */
function handStrengthBucket(cards: Card[]): string {
  ...
  if (maxGroup >= 3 || pairCount >= 2) return 'vStr';  // trips+, two pair
  if (maxGroup >= 2) return 'pair';                     // one pair
  if (maxSuit >= 3 || maxConsecutive >= 3) return 'draw'; // flush or straight draw
...
function handVsClaimBucket(myCards: Card[], currentHand: HandCall | null): string {
  ...
  switch (currentHand.type) {
    ...
    case HandType.PAIR: {
      ...
      const hasSomePair = hasPair(myCards);
...
/**
 * Generate a compact info set key for 5 Draw CFR.
 *
 * Format: fd|<position>|<handStrength>|<handVsClaim>|<claimHeight>|<turnDepth>
 *
 * Since 5 Draw is always 2 players, 5 cards each, 10 total, we don't need
 * player count or total cards buckets.
 */
